...
def Geometric_Brownian_Motion(S_0, mu, T, dt, n_paths, v0, kappa, theta, sigma_v):
    """
    Simulates stock prices according to the Geometric Brownian Motion with stochastic volatility (Heston model).
    ...
    v0      : float
        Initial variance (Heston model parameter).
    kappa   : float
        Mean reversion rate (Heston model parameter).
    theta   : float
        Long-term mean variance (Heston model parameter).
    sigma_v : float
        Volatility of variance (vol of vol) (Heston model parameter).

    Returns:
    stock_paths : np.ndarray
        Simulated stock price paths (shape: n_paths x n_steps + 1).
    vol_paths   : np.ndarray
        Simulated volatility paths (shape: n_paths x n_steps + 1).
    """
    n_steps = int(T / dt)
    stock_paths = np.zeros((n_paths, n_steps + 1))
    stock_paths[:, 0] = S_0  # Set initial stock price

    # Generate volatility paths using the Heston model
...
stock_paths, vol_paths = Geometric_Brownian_Motion(S0, gbm_params['mu'], T, dt, n_paths, high_vol_params['v0'], high_vol_params['kappa'], high_vol_params['theta'], high_vol_params['sigma_v'])
print(vol_paths)
plt.figure(figsize=(12, 7))
for i in range(stock_paths.shape[0]):
    plt.plot(stock_paths[i, :], alpha=0.7)
...
def simulate_stock_prices(S0, T, dt, n_paths, high_vol_params, low_vol_params, future_volatility):
    """
    Simulates stock prices using the Heston model for high volatility and the Geometric Brownian Motion (GBM) for low volatility
    based on predicted regimes.
    ...
    n_paths         : int
        Number of simulation paths.
    high_vol_params : dict
        Parameters for the Heston model (v0, kappa, theta, sigma_v).
    low_vol_params  : dict
        Parameters for GBM (mu).
    future_volatility : list or array
        Predicted regimes for each time step ("High Volatility" or "Low Volatility").
        
    Returns:
    simulated_prices : pd.DataFrame
        Simulated stock prices for each path over time.
    """
    n_steps = int(T / dt)
    simulated_prices = np.zeros((n_paths, n_steps + 1))
    simulated_prices[:, 0] = S0  # Set initial stock price for all paths

    for t, regime in enumerate(future_volatility):
        print(f"Step {t}, Regime: {regime}")

        if regime == "High Volatility":
            # Use the Heston model for high volatility
            vol_paths = heston_volatility_process(
                v0=high_vol_params["v0"], 
                kappa=high_vol_params["kappa"], 
                theta=high_vol_params["theta"], 
                sigma_v=high_vol_params["sigma_v"], 
                T=dt,  # Simulate for one time step
                dt=dt, 
                n_paths=n_paths
            )
            Z = np.random.normal(size=n_paths)  # Random noise
            simulated_prices[:, t+1] = simulated_prices[:, t] * np.exp(
                (-0.5 * vol_paths[:, 1] ** 2) * dt + vol_paths[:, 1] * Z * np.sqrt(dt)
            )

        elif regime == "Low Volatility":
            # Use the GBM model for low volatility
            stock_paths, _ = Geometric_Brownian_Motion(
                S_0=simulated_prices[:, t], 
            ...
            simulated_prices[:, t+1] = stock_paths[:, 1]
...
# High-volatility parameters (Heston)
high_vol_params = {
    "v0": 0.04,
    "kappa": 2.0,
    "theta": 0.04,
...
# Low-volatility parameters (GBM)
low_vol_params = {
    "mu": 0.05  # Drift
}

# Example future volatility regimes (alternating high and low volatility)
future_volatility = ["High Volatility" if i < 50 else "Low Volatility" for i in range(int(T / dt))]

# Simulate stock prices
simulated_prices = simulate_stock_prices(S0, T, dt, n_paths, high_vol_params, low_vol_params, future_volatility)
...
plt.title("100 Simulations of Stock Prices with Regime Switching")
plt.ylabel("Price in $")
plt.xlabel("Time Steps")
plt.show()
